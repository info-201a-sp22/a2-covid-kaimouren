...
states <- states %>% mutate(ratio = deaths/cases)
state_highest_ratio <- states %>% filter(ratio == max(ratio)) %>% pull(state)
...
state_lowest_cases <- states %>% filter(date == "2022-04-26") %>% filter(cases == min(cases)) %>% pull(state)
...
number_highest_cases_county <- counties %>% filter(cases == max(cases)) %>% pull(cases)
...
counties <- counties %>% mutate(location = paste(county, state))
...
# As you have seen, the three datasets are "cumulative sums" â€? that is, running totals of the number of cases and deaths. On each day, the cases and deaths counts either stay the same or increase. However, it would be helpful to know how much cases or deaths increase each day.

# 2.k Add a new column to your `national` data frame called `new_cases` â€? that is, the number new cases each day.
...
highest_cases_in_each_state <- counties %>% group_by(state) %>% filter(date == max(date)) %>% filter(cases == max(cases))
...
lowest_deaths_in_each_state <- counties %>% group_by(state) %>% filter(date == max(date)) %>% filter(deaths == max(deaths))
...
# As described on the New York Times GitHub page, collecting this data has been a massive effort. Accordingly, there might be mistakes in the data â€? perhaps especially in the counties dataset, where data is being collected for more than 3,000 U.S. counties.
...
all_totals <- left_join(total_cases_counties, national, by = c('date' = 'date'))
...
#which state has the lowest death rate in most recent day>

#  Write code (at least 2-3 lines) that will answer your question
my_answer <- states %>% filter(date == max(date)) %>% filter(ratio == min(ratio)) %>% pull(state)
